...
export class EducadorCadastroComponent implements OnInit {
  ...
  constructor(
    ...
    if(this.perfil.toString() == "Educador")this.educador._id = localStorage.getItem("user");
    if(this.perfil.toString() == "Admin") this.educador._id = this.route.snapshot.paramMap.get("id");
  ...
  cadastro() {
    if (this.verificaSenha()) {
      this.educadorService.cadastro(this.educador).subscribe(res => {
